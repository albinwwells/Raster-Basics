...
def scatterplot(data, xtitle, ytitle, title, colors, labels, markers, xyLine=False, buff=100):
    fig = plt.figure()
    ...
    fig.savefig('Figures/' + figName, dpi=250)


def plotMany(cbarTitle, color, plotTitle, *kwargs):
    # All *kwargs should follow this: [data, title, v] where v is [vmin, vcenter, vmax]
    ...
    # plotMany('Velocity (m/yr)', 'BrBG', 'Plot of All IcePack Results', 0, 5, 60,
    #          list(zip(icepack_v_list, icepack_title_list)))
...
def plotData(dataVals, cbarTitle, color, plotTitle, cluster=None, quiver=None, colorbarMax=None):
    '''
...
def plotData3(dataVals1, cbarTitle1, color1, title1, dataVals2, cbarTitle2, color2, title2,
              ...
              div_select=[0,0,0], min=[0,0,0], mean=[50,50,50], max=[100,100,100]):
...
def plotClassify(array, class_vals, class_labels, colors, title):
...
def plotContinuous(array, range, cbar, title, cbarTitle, quiver=None):
